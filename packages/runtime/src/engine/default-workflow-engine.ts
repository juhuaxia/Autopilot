...
export class DefaultWorkflowEngine implements WorkflowEngine {
  ...
  private buildArtifactRepairPromptLines(args: {
    ...
    lines.push("[ARTIFACT_SECTION_ORDER_POLICY] Do not add, remove, rename, or reorder artifact section headings. Preserve the template heading order exactly. Only edit section bodies. If the current artifact has headings out of order, rebuild the artifact using the standard template order before filling content.")
  ...
  private async buildDispatchPrompt(workflowId: string, phase: string, reason: string): Promise<string> {
    ...
    if (phase === "test") {
      lines.push("[TEST_QUALITY_POLICY] Execute the most relevant available validation commands when feasible and base PASS/FAIL on actual evidence, not assumption.")
      lines.push("[TEST_POLICY] Validate the implementation and review findings. This phase is read-only for project code: do not modify application code, tests, routes, APIs, assets, configs, generated files, snapshots, fixtures, or any non-test artifact files. Run or inspect verification only; if a failure requires code or test changes, mark FAIL and record the required fix instead of applying it. Ensure testing covers both the requested behavior and any affected upstream/downstream files, parent-controlled flows, shared dependencies, and previously working functionality touched by the traced impact boundary. Any review must include at least one runnable verification path when the stack supports it (build, compile, typecheck, test, or startup). Blocking build/compile/typecheck/test/startup failures must be marked FAIL. Update the test artifact with executed checks, failures, regression evidence for impacted existing features, coverage summary, and set explicit pass/fail conclusion. Unverified content must be recorded as unverified or evidence insufficient, not pass. Current code and current verification results are the source of truth; historical artifacts may inform scope but never substitute for evidence. Keep section headings unchanged.")
      lines.push("[TEST_SECTION_ORDER_POLICY] Preserve the test artifact heading order exactly as the current template. Do not move ## 结论 above required evidence sections. Only edit section bodies.")
